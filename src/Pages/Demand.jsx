...
const Demand = () => {
  ...
  const fetchDemands = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/demand`);
  ...
  const updateCandidatesToNotInProgress = async (demandId) => {
    try {
      ...
      const response = await axios.put(`${API_BASE_URL}/api/selected-candidates/demand/${demandId}/update-status`);
  ...
  const removeAllSelectedCandidates = async (demandId) => {
    try {
      ...
      const response = await axios.delete(`${API_BASE_URL}/api/selected-candidates/demand/${demandId}/all`);
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      const response = await axios.put(`${API_BASE_URL}/api/selected-candidates/status`, requestBody);
  ...
  const handleCandidateAction = async (candidateId, newStatus, reason) => {
    try {
      ...
      const response = await axios.put(`${API_BASE_URL}/api/selected-candidates/status`, {
  ...
  const handleCreateDemand = async () => {
    try {
      ...
      toast.error("Failed to create demand: " + err.message);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDeleteDemand = async (demandId) => {
    confirmAction("Are you sure you want to delete this demand?", async () => {
      setIsDeletingDemand(true);
      try {
        const response = await fetch(`${API_BASE_URL}/api/demand/${demandId}`, {
          method: "DELETE",
          headers: {
            "Accept": "application/json"
          }
        ...
        toast.error("Failed to delete demand: " + err.message);
      } finally {
  ...
  const handleSaveDemand = async () => {
    ...
    if (!isAdding && editedDemand?.status !== previousStatus) {
      if (!statusChangeDesc.trim()) {
        toast.error("Please enter a reason for status change");
        return;
      }
    }
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/demand/${editedDemand.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const updateCandidateStatus = async (newStatus) => {
    ...
    try {
      ...
      const statusResponse = await axios.put(
        `${API_BASE_URL}/api/selected-candidates/status-with-zone`,
        statusRequest
      );
      ...
      if (statusResponse.data.success) {
        ...
        if (shouldBeInProgress) {
          ...
          await axios.put(
            `${API_BASE_URL}/api/candidates/${selectedStatusCandidate.id}/progress`,
            { isInProgress: true }
          );
          // Remove candidate from Zone when moving back to active status
          try {
            await axios.delete(
              `${API_BASE_URL}/api/zone/remove/${selectedStatusCandidate.id}/${selectedDemandDetails?.clientName}`
            );
        ...
        if (isJoinedStatus) {
          toast.success(`Candidate marked as JOINED! Profile is no longer "In Progress".`);
        } else if (shouldGoToZone) {
          toast.success(`Candidate marked as ${newStatus} and added to Zone.`);
        } else {
                    ...
                    <>
                      <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors
                              ${isSaving ? "bg-gray-400 cursor-not-allowed" : "bg-green-500 hover:bg-green-600 text-white cursor-pointer"}`}
                        title="Save"
                      >
                        {isSaving ? (
                          <>
                    ...
                    <>
                      ...
                      {userRole === "Admin" && (
                        <button
                          ...
                          className={`flex items-center gap-2 px-4 py-2 text-white rounded-lg transition-colors cursor-pointer ${isDeletingDemand ? 'bg-gray-400 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700'}`}
                          title="Delete"
                        >
                          {isDeletingDemand ? <Loader className="animate-spin" size={18} /> : <Trash2 size={18} />}
                          {isDeletingDemand ? "Deleting..." : "Delete"}
                        </button>
                      )}
                      <button
                        ...
                        <div className="mb-3">
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            {candidate.resumePath || candidate.googleDriveViewLink ? (
                              <a
                                href={candidate.googleDriveViewLink || `${API_BASE_URL}${candidate.resumePath}`}
                                target="_blank"
