...
const AdminMyclient = () => {
  ...
  const fetchAllocations = async () => {
    try {
      const url = searchTerm ? `${API_BASE_URL}/api/allocations?search=${encodeURIComponent(searchTerm)}` : `${API_BASE_URL}/api/allocations`;
  ...
  const fetchMetrics = async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/allocations/metrics`);
      if (res.data.success) {
  ...
  const fetchEmployees = async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/personal-details`);
      if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`${API_BASE_URL}/api/allocations/${editingId}`, formData);
        toast.success("Allocation updated successfully!");
      } else {
        await axios.post(`${API_BASE_URL}/api/allocations`, formData);
      ...
      toast.error("Error saving allocation: " + (err.response?.data?.message || err.message));
    }
  };

  const handleDelete = async (id) => {
    confirmAction("Are you sure you want to delete this allocation?", async () => {
      try {
        await axios.delete(`${API_BASE_URL}/api/allocations/${id}`);
