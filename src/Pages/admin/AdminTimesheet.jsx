...
const AdminTimesheet = () => {
  ...
  const fetchTimesheet = async (uid) => {
    setLoading(true);
    try {
      const res = await axios.get(`http://localhost:5000/api/timesheet/user/${uid}?month=${monthStr}`);
      if (res.data.success) {
  ...
  const fetchTeamList = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`http://localhost:5000/api/timesheet/all?month=${monthStr}`);
      if (res.data.success) {
  ...
  const saveException = async () => {
    if (!exceptionReason.trim()) {
      alert("Reason is required when overriding hours.");
      return;
    }
    try {
      await axios.post("http://localhost:5000/api/timesheet/exception", {
        userId: selectedUser,
  ...
  const handleStatusUpdate = async (newStatus) => {
    try {
      await axios.post("http://localhost:5000/api/timesheet/status", {
        userId: selectedUser,
        monthStr: monthStr,
        status: newStatus,
  ...
  const canEdit = timesheetData?.status !== "Locked";
  const canApprove = timesheetData?.status !== "Approved" && timesheetData?.status !== "Locked";
  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex gap-4 mb-6 border-b border-gray-200">
        <button
          className="pb-3 px-2 font-medium text-blue-600 border-b-2 border-blue-600"
        >
          All Employees
        </button>
      </div>
