...
export default function LeaveManagement() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (company) {
        try {
          const holidayRes = await fetch(`${GLOBAL_API_BASE_URL}/api/holiday/group/${encodeURIComponent(company)}`);
  ...
  const checkSupervisorAndFetchTeamLeaves = async (userId) => {
    try {
      ...
      if (role === 'HR') {
        ...
        const teamRes = await axios.get(`${GLOBAL_API_BASE_URL}/api/teams/user/${userId}`);
  ...
  const handleUpdateLeaveStatus = async (id, newStatus) => {
    confirmAction(`Are you sure you want to mark this request as ${newStatus}?`, async () => {
      try {
        const storedUser = JSON.parse(localStorage.getItem('user'));
        const role = storedUser?.role;
