...
const MyTeam = () => {
  ...
  useEffect(() => {
    const fetchTeamData = async () => {
      ...
      try {
        const res = await axios.get(`${API_BASE_URL}/api/teams/user/${username}`);
        if (res.data.success) {
  ...
  const handleLeaveAction = async (leaveId, status) => {
    ...
    try {
      const res = await axios.put(`${API_BASE_URL}/api/leave/status/${leaveId}`, {
        status,
        remarks: ''
      });
      if (res.data.success) {
                      ...
                      <div className="flex items-center gap-4 w-full">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-col mt-1.5 space-y-1.5">
                            <p className="text-xs text-gray-500 font-mono bg-gray-50 px-2 py-0.5 rounded border border-gray-100 w-fit max-w-full truncate">
                              <span className="text-gray-400 font-sans text-[10px] uppercase tracking-wider">ID:</span> {memberTeam.supervisor.employeeNumber || memberTeam.supervisor.userId}
                            </p>
                            {memberTeam.supervisor.email && (
                              <div className="flex items-start gap-1.5 text-xs text-emerald-600 min-w-0">
                                <Mail size={12} className="flex-shrink-0 mt-0.5" />
                                <a href={`mailto:${memberTeam.supervisor.email}`} className="hover:text-emerald-700 hover:underline break-all transition-colors" title={memberTeam.supervisor.email}>
                                  {memberTeam.supervisor.email}
                                </a>
                              </div>
                            )}
                    ...
                    {memberTeam.hr ? (
                      <div className="flex items-center gap-4 w-full">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-col mt-1.5 space-y-1.5">
                            <p className="text-xs text-gray-500 font-mono bg-gray-50 px-2 py-0.5 rounded border border-gray-100 w-fit max-w-full truncate">
                              <span className="text-gray-400 font-sans text-[10px] uppercase tracking-wider">ID:</span> {memberTeam.hr.employeeNumber || memberTeam.hr.userId}
                            </p>
                            {memberTeam.hr.email && (
                              <div className="flex items-start gap-1.5 text-xs text-purple-600 min-w-0">
                                <Mail size={12} className="flex-shrink-0 mt-0.5" />
                                <a href={`mailto:${memberTeam.hr.email}`} className="hover:text-purple-700 hover:underline break-all transition-colors" title={memberTeam.hr.email}>
                                  {memberTeam.hr.email}
                                </a>
                              </div>
                            )}
                  ...
                  {memberTeam.members && memberTeam.members.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {memberTeam.members.map(member => {
                        ...
                        return (
                          <div key={member.userId} className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${isCurrentUser
                              ? 'border-blue-200 bg-blue-50/50 shadow-sm'
                              : 'border-gray-100 hover:border-blue-200 hover:bg-blue-50/30'
                            }`}>
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 font-bold shadow-sm border overflow-hidden ${isCurrentUser
