...
import DashboardLayout, { DashboardContainer } from '../../components/dashboard/DashboardLayout.jsx';
...
const MyAssets = () => {
  ...
  const fetchPersonalDetailsAndAssets = async (username) => {
    ...
    try {
      ...
      try {
        const pdRes = await axios.get(`${GLOBAL_API_BASE_URL}/api/personal-details?userId=${username}`);
        ...
        try {
          const altRes = await axios.get(`${GLOBAL_API_BASE_URL}/api/personal-details/${username}`);
  ...
  const removeAsset = (id) => {
    if (assets.length === 1) {
      toast.error("You must have at least one asset entry");
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!hasValidAsset) {
      toast.error("Please enter at least one asset with quantity");
      return;
    }
    ...
    for (const asset of assets) {
      if (asset.asset_name && asset.quantity && parseInt(asset.quantity) > 0) {
        ...
        if (asset.asset_name === "Other" && !asset.custom_asset.trim()) {
          toast.error("Please specify the asset name when selecting 'Other'");
          return;
        }
        filteredAssets.push({
    ...
    if (filteredAssets.length === 0) {
      toast.error("Please fill at least one asset with name and quantity");
      return;
    }
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Failed to save assets");
    } finally {
      setSubmitting(false);
    }
  };
